...
async def on_fa2_token1_to_fa2_token2(
    ctx: HandlerContext,
    ...
    for index, tx in enumerate(transfer_0.parameter.__root__[0].txs):
        token_1_id = tx.token_id
        if token1_id == token_1_id:
            tx_token1_quantity = Decimal(tx.amount) / (10 ** decimals_1)
            tx_token2_quantity = Decimal(transfer_1.parameter.__root__[0].txs[index].amount) / (10 ** decimals_2)
